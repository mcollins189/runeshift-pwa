...
function buildBundle() {
  ...
  // Which fakemon actually HAVE drawn artwork, as "<game>/<id>" keys.
  //
  // The detail panel prefers sprites/hack-art/<game>/<id>.webp over the 64x64 ROM
  // sprite, but most hacks have no artwork at all and many that do only cover part of
  // their dex. Pointing the panel at a file that is not there left it showing NOTHING
  // for every Pisces fakemon — the onerror fallback did not rescue it in practice. So
  // the app asks this manifest first and never requests a file that does not exist.
  const hackArt = [];
  {
    const artRoot = join(SCRIPT_DIR, 'sprites', 'hack-art');
    if (existsSync(artRoot)) {
      for (const game of readdirSync(artRoot)) {
        const dir = join(artRoot, game);
        if (!statSync(dir).isDirectory()) continue;
        for (const f of readdirSync(dir)) {
          const m = f.match(/^(\d+)\.webp$/);
          if (m) hackArt.push(`${game}/${m[1]}`);
        }
      }
    }
  }

  const payload = { meta, routes, leagues, patches, encounterTables, learnsets, dexIdBySlug, formIdBySlug, typesBySlug, slugAliases, validApi, regionSpeciesIndex, moveStats, evoBySlug, statsBySlug, hackArt };
